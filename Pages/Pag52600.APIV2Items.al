page 52600 "DME APIV2 - Items"
{
    APIVersion = 'v2.0';
    EntityCaption = 'SMBItem';
    EntitySetCaption = 'SMBItems';
    EntityName = 'SMBitem';
    EntitySetName = 'SMBitems';
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field(CRMDes; CRMDescripton)
                {
                    Caption = 'CRM Description';
                    Editable = false;
                }
            }
        }
    ...
    local procedure GetCRMDescription()
    var
        MyInStream: InStream;
    begin
        Clear(CRMDescripton);
        Rec.Calcfields("Item CRM Description");
        If Rec."Item CRM Description".HasValue() then begin
            Rec."Item CRM Description".CreateInStream(MyInStream);
            MyInStream.Read(CRMDescripton);
        end;
    end;
}
